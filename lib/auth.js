...
export const ROLES = Object.freeze({ FIDELITY: 'fidelity', EMPLOYEE: 'employee', MANAGER: 'manager', ADMIN: 'admin' });
export const ROLE_LABELS = Object.freeze({ fidelity: 'Fidélité', employee: 'Employé', manager: 'Responsable', admin: 'Administrateur' });
const ROLE_LEVEL = Object.freeze({ fidelity: 0, employee: 1, manager: 2, admin: 3 });
...
function configuredAccounts() {
  return [
    { username: process.env.FIDELITY_USERNAME || 'fidelite', password: process.env.FIDELITY_PASSWORD, role: ROLES.FIDELITY },
...
export function getSession(request) {
  ...
  try {
    ...
    return session.exp > Date.now() && Object.prototype.hasOwnProperty.call(ROLE_LEVEL, session.role) ? session : null;
  } catch {
    return null;
  }
}

...
async function sendToRows(rows,{title,body,url='/',tag='lbdc'}){
...
export async function sendNotification(payload) { return sendToRows(await sb(`${tables.subscriptions}?select=*&limit=10000`),payload) }
export async function sendMemberNotification(memberId,payload){return sendToRows(await sb(`${tables.subscriptions}?member_id=eq.${encodeURIComponent(memberId)}&select=*&limit=100`),payload)}

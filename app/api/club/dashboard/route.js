...
export async function GET(request) {
  ...
  try {
    const [members, history, promos, subs] = await Promise.all([
      sb(`${tables.members}?select=id,loyalty_points&limit=10000`),
      sb(`${tables.history}?select=*&order=created_at.desc&limit=10000`),
      sb(`${tables.promotions}?select=id&active=eq.true`),
      sb(`${tables.subscriptions}?select=id&limit=10000`),
    ]);
    return NextResponse.json({ configured: true, members: members.length, stamps: members.reduce((total, member) => total + (Number(member.loyalty_points) || 0), 0), rewards: history.filter(item => item.event_type === 'reward').length, promotions: promos.length, subscribers: subs.length, recent: history.slice(0,20) });

...
import { upcomingBirthday } from '../../../../lib/birthday-offers';
...
export async function GET(request){
 ...
 try{
  ...
  const memberRows=members.map(member=>{const visits=passagesByMember.get(member.id)||[],lastPassage=visits.reduce((latest,event)=>eventTime(event)>eventTime(latest)?event:latest,visits[0]||null),referenceTime=lastPassage?eventTime(lastPassage):new Date(member.created_at).getTime(),inactiveDays=Number.isFinite(referenceTime)?Math.max(0,Math.floor((nowTime-referenceTime)/86400000)):0;return{id:member.id,name:[member.first_name,member.last_name].filter(Boolean).join(' ')||member.email||member.personal_code,firstName:member.first_name||'',email:member.email||'',birthday:member.birthday||null,nextBirthday:upcomingBirthday(member.birthday,now),lastPassageAt:lastPassage?.created_at||lastPassage?.event_day||null,totalPassages:visits.length,inactiveDays,createdAt:member.created_at,stamps:Number(member.loyalty_points)||0,rewardReached:program.enabled&&(Number(member.loyalty_points)||0)>=program.threshold,passages30Days:visits.filter(event=>eventTime(event)>=thirtyDaysAgo).length}});
